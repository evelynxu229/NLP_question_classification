...
class TextParser():
    ...
    def remove_stopwords(self, sentence):
        ...
        result_words = [word for word in words if word.lower()
    ...
    def create_vocab(self, to_file):
        if len(self.vocab) == 0:
            if len(self.words) > 0:
                ...
                self.vocab = sorted(self.vocab_count_dict,
                                    key=self.vocab_count_dict.get, reverse=True)
                self.vocab.insert(0, '')
    ...
    def create_label(self, to_file):
        if len(self.fine_labels) == 0:
            if len(self.fine_pair) > 0:
                ...
                self.fine_labels = sorted(
                    fine_labels, key=fine_labels.get, reverse=True)
        if len(self.fine_labels) > 0:
            if to_file:
        ...
        if len(self.coarse_labels) == 0:
            if len(self.coarse_pair) > 0:
                ...
                self.coarse_labels = sorted(
                    coarse_labels, key=coarse_labels .get, reverse=True)
        if len(self.coarse_labels) > 0:
            if to_file:
